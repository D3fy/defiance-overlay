...
# ngx-http-auth-spnego-module (https://github.com/dmolik/ngx_http_auth_spnego, BSD-2)
HTTP_SPNEGO_MODULE_PV="0.9.0"
HTTP_SPNEGO_MODULE_P="ngx_http_auth_spnego-${HTTP_SPNEGO_MODULE_PV}"
HTTP_SPNEGO_MODULE_URI="https://github.com/dmolik/ngx_http_auth_spnego/archive/v${HTTP_SPNEGO_MODULE_PV}.tar.gz"
...
SRC_URI="https://nginx.org/download/${P}.tar.gz
	...
	nginx_modules_http_auth_spnego? ( ${HTTP_SPNEGO_MODULE_URI} -> ${HTTP_SPNEGO_MODULE_P}.tar.gz )
...
src_configure() {
	...
	if use nginx_modules_http_auth_spnego; then
		http_enabled=1
		myconf+=( --add-module=${HTTP_SPNEGO_MODULE_WD} )
	fi
