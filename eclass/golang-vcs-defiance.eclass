...
# @ECLASS: golang-vcs-defiance.eclass
# @MAINTAINER:
...
_golang-vcs_env_setup() {
	...
	if [[ ! -d ${EGO_STORE_DIR} ]]; then
		(
			addwrite /
			mkdir -p "${EGO_STORE_DIR}/${P}"
		) || die "${ECLASS}: unable to create ${EGO_STORE_DIR}/${P}"
	fi

	addwrite "${EGO_STORE_DIR}/${P}"
...
_golang-vcs_fetch() {
	...
	if [[ -z ${EVCS_OFFLINE} ]]; then
		...
		set -- env GOPATH="${EGO_STORE_DIR}/${P}" go get -d -t -u -v -x "${EGO_PN}"
		echo "$@"
		"$@" || die
	...
	local go_srcpath="${WORKDIR}/${P}/src"
	set -- mkdir -p "${go_srcpath}"
	echo "$@"
	"$@" || die "Unable to create ${go_srcpath}"
	set -- cp -r	"${EGO_STORE_DIR}/${P}/src/" \
		"${go_srcpath}/.."
	echo "$@"
