...
BUILD_ID="3.7.1"
ECLIPSE_BUILD_VER="bd5e22fe1c5897603235f7da0d6db31035ab270b"
...
SRC_URI="http://download.eclipse.org/technology/linuxtools/eclipse-build/${SLOT}/eclipse-${BUILD_ID}-src.tar.bz2
	http://download.eclipse.org/technology/linuxtools/eclipse-build/${SLOT}/eclipse-build-${ECLIPSE_BUILD_VER}.tar.xz"



CDEPEND=">=dev-java/swt-${PV}:3.7
	>=dev-java/ant-1.7.1
...
buildDir="${S}/build/eclipse-${BUILD_ID}-src"
...
src_unpack() {
	...
	mv "${WORKDIR}/eclipse-build-${ECLIPSE_BUILD_VER}" "${S}" || die
	ln -s "${DISTDIR}/eclipse-${BUILD_ID}-src.tar.bz2" "${S}"/ || die
...
src_prepare() {
	# epatch "${FILESDIR}/${SLOT}/jvmarg.patch" || die
